...
const Profile = () => {
  ...
  return (
    <div className="relative -z-10 pt-28 pb-10 px-3  flex items-center justify-center bg-cover bg-center bg-gray-100">
      <div className="profile-card w-[300px]  rounded-md shadow-xl overflow-hidden z-[100] relative cursor-pointer snap-start shrink-0 bg-white flex flex-col items-center justify-center gap-3 transition-all duration-300 group">
        <div className="avatar w-full pt-5 flex items-center justify-center flex-col gap-1">
          <div className="img_container w-full flex items-center justify-center relative z-40 after:absolute after:h-[6px] after:w-full after:bg-teal-600 after:top-4 after:group-hover:size-[1%] after:delay-300 after:group-hover:delay-0 after:group-hover:transition-all after:group-hover:duration-300 after:transition-all after:duration-300 before:absolute before:h-[6px] before:w-full before:bg-teal-600 before:bottom-4 before:group-hover:size-[1%] before:delay-300 before:group-hover:delay-0 before:group-hover:transition-all before:group-hover:duration-300 before:transition-all before:duration-300">
            <PiUserCircleDuotone size={150} className="fill-white z-50" />
            <div className="absolute bg-teal-600 z-10 size-[60%] w-full group-hover:size-[1%] group-hover:transition-all group-hover:duration-300 transition-all duration-300 delay-700 group-hover:delay-0"></div>
          </div>
        </div>
        <div className="headings *:text-center *:leading-4 py-3">
          <p className="text-xl font-serif font-semibold text-[#434955]">
            {userLogin?.user?.nombre} {userLogin?.user?.apellido}
          </p>
          ...
          <ul className="flex flex-col items-start gap-2 has-[:last]:border-b-0 *:inline-flex *:gap-2 *:items-center *:justify-center *:border-b-[1.5px] *:border-b-stone-700 *:border-dotted *:text-xs *:font-semibold *:text-[#434955] pb-3">
            {/* <li>
              <svg
                id="phone"
                viewBox="0 0 24 24"
                className="fill-stone-700 group-hover:fill-teal-600"
                height="15"
                width="15"
            ...
            <li>
              ...
              <p>{userLogin?.user?.correoElectronico}</p>
            </li>
            <li>
              <svg
                className="fill-stone-700 group-hover:fill-teal-600"
        ...
        <hr className="w-full group-hover:h-5 h-3 bg-teal-600 group-hover:transition-all group-hover:duration-300 transition-all duration-300" />
      </div>
      <GlobalWhatsAppContactButton message="Tengo algunas preguntas sobre mi perfil. ¿Pueden ayudarme?" />
    </div>
  );
};
